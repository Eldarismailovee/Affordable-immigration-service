...
export async function generateOnboardingPacketForLead(leadId) {
  ...
  try {
    ...
    if (existingPacket.rows.length > 0) {
      return {
        alreadyExists: true,
        onboarding: existingPacket.rows[0],
      };
    }
    ...
    const packet = generateOnboardingPacket(payload);
    ...
    const insertResult = await client.query(
      ...
      `,
      [onboardingId, leadId, packet.title, packet.html, "generated"]
    );

    return {
      alreadyExists: false,
      onboarding: insertResult.rows[0],
    };
  } finally {
    client.release();
  }
